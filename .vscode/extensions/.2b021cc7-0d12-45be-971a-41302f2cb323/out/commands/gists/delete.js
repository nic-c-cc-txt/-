...
var deleteCommand = function (_config, services, utils) {
    var gists = services.gists, insights = services.insights, logger = services.logger;
    var command = extension_commands_1.GistCommands.Delete;
    var closeGistEditors = function (gistId) {
        var editors = vscode_1.window.visibleTextEditors;
        editors.forEach(function (e) {
            var id = utils.files.extractTextDocumentDetails(e.document).id;
            if (gistId === id) {
                vscode_1.window.showTextDocument(e.document);
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    id = utils.files.extractTextDocumentDetails(doc).id;
                    if (!id) return [3, 2];
                    logger.info("Deleting Gist \"" + id + "\"");
                    return [4, gists.deleteGist(id)];
                case 1:
                    _a.sent();
                    closeGistEditors(id);
