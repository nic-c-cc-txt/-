...
class FolderTaskProvider {
    ...
    isEnabled() {
        return vscode.workspace
            .getConfiguration("healthier", this._workspaceFolder.uri)
            .get("provideLintTask");
    }
    start() { }
    dispose() { }
    getTask() {
        return __awaiter(this, void 0, void 0, function* () {
            let rootPath = this._workspaceFolder.uri.scheme === "file"
            ...
            try {
                let command = yield findHealthier(rootPath);
                let kind = {
                    type: "healthier"
                };
                let options = {
                ...
                return new vscode.Task(kind, this.workspaceFolder, "lint whole folder", "healthier", new vscode.ShellExecution(`${command}`, options), "$eslint-stylish");
            }
            catch (error) {
                return undefined;
            }
...
class TaskProvider {
    ...
    updateProvider() {
        if (!this.taskProvider && this.providers.size > 0) {
            this.taskProvider = vscode.workspace.registerTaskProvider("healthier", {
                provideTasks: () => {
