...
const interceptor_1 = require("@manuth/interceptor");
...
class Plugin {
    ...
    Decorate(languageService) {
        ...
        if (!languageService[Constants_1.Constants.PluginInstalledSymbol] &&
            ...
            interceptor.AddProperty(Constants_1.Constants.PluginInstalledSymbol, {
                Get: () => true
            });
    ...
    InstallInterceptions(interceptor) {
        interceptor.AddMethod("getSemanticDiagnostics", (target, delegate, fileName) => {
            ...
            if (!this.Config.SuppressWhileTypeErrorsPresent || (diagnostics.length === 0)) {
                ...
                for (let diagnostic of report) {
                    if (diagnostic instanceof ConfigNotFoundMessage_1.ConfigNotFoundMessage) {
                        if (!this.Config.SuppressConfigNotFoundError) {
                            result.push(diagnostic);
                        }
                    }
                    else if (diagnostic instanceof DeprecationMessage_1.DeprecationMessage) {
                        if (!this.Config.SuppressDeprecationWarnings) {
                            result.push(diagnostic);
                        }
                    }
                    else if (diagnostic instanceof ESLintDiagnostic_1.ESLintDiagnostic) {
                        if (diagnostic.LintMessage.severity > 0) {
                            ...
                            documentDiagnostics.Set(diagnostic.Parsed.start, diagnostic.Parsed.start + diagnostic.Parsed.length, {
        ...
        interceptor.AddMethod("getCodeFixesAtPosition", (target, delegate, fileName, start, end, errorCodes, formatOptions, userPreferences) => {
            var _a, _b, _c;
            (_a = this.Logger) === null || _a === void 0 ? void 0 : _a.Verbose(`Code-fixes requested inside of \`${fileName}\` from offset ${start} to ${end}`);
